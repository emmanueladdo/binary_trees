...
/**
 * right_node - node value instertion in a AVL.
 ...
 */
avl_t *right_node(avl_t **tree, avl_t *parent, avl_t **new, int nval)
{
	int bval;
	...
	if ((*tree)->n > nval)
	{
		(*tree)->left = right_node(&(*tree)->left, *tree, new, nval);
		if ((*tree)->left == NULL)
			return (NULL);
	}
	else if ((*tree)->n < nval)
	{
		(*tree)->right = right_node(&(*tree)->right, *tree, new, nval);
		if ((*tree)->right == NULL)
			return (NULL);
	}
	else
...
avl_t *avl_insert(avl_t **tree, int value)
{
	...
	right_node(tree, *tree, &new, value);
	return (new);
}
